...
static bool isKvText(const std::string& text) {
...
std::string ResponseParser::parseBulkString(int socketFd) {
    ...
    while (totalRead < static_cast<size_t>(len)) {
        ...
        if (r <= 0) return "{\"error\": \"Failed to read bulk string\"}";
        totalRead += r;
    }
    ...
    if (value.rfind("# Server", 0) == 0 || isKvText(value)) {
...
std::string ResponseParser::parseVerbatimString(int socketFd) {
    ...
    while (totalRead < static_cast<size_t>(len)) {
        ...
        if (r <= 0) return "{\"error\": \"Failed to read verbatim string\"}";
        totalRead += r;
    }
...
std::string ResponseParser::parseRawResponse(int socketFd) {
    ...
    switch (prefix) {
        ...
        default:  return "{\"error\": \"Unknown response type: " + std::string(1, prefix) + "\"}";
    }
}
